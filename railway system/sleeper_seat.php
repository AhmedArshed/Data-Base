 ...
 <?php
    if (isset($_POST["tdate"])) {
        ...
        while($count <= $Noofseats){
            $qry = "INSERT into train_seats values('".$box."','".$seat."','".$date."','".$tid."','".$cid."')";
            
            if($con->query($qry)){
                $count = $count + 1;
                if($count != $Noofseats){
                    $msg = "Seat Book";
                    header("Location:sleeper_seat.php?Message=$msg && tid=$tid && cid=$cid && count=$count");
                }
                if($count == $Noofseats){
                ...
                //("Location:sleeper_seat.php?Message=$msg && tid=$tid && cid=$cid && count=$count");
            }
        }
    }
?>
